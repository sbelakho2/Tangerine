...
public final class MIRInterpreter {
    ...
    private func evalRvalue(_ rvalue: MirRvalue) -> MirValue {
                ...
                if let tdIndices = typeDefIndex[name] {
                    ...
                    for tdIdx in tdIndices {
                        ...
                        if hasNamedPayloadStruct {
                            let payloadVals = vals.count > 1 ? Array(vals.dropFirst()) : vals
                            let payloadStruct = payloadVals.first ?? .unit
                            resolvedPayload = payloadStruct
                        } else {
                            let fieldCount = variants[idx].1.count
                            let payloadVals = vals.count > fieldCount ? Array(vals.dropFirst()) : vals
                            if fieldCount == 0 {
                                resolvedPayload = .unit
                            } else if fieldCount == 1 {
                                resolvedPayload = payloadVals.first ?? .unit
                            } else {
                                resolvedPayload = .tuple(payloadVals)
                            }
                        }
                        break
                    }
                    payload = resolvedPayload ?? (vals.isEmpty ? .unit : .tuple(vals.count > 1 ? Array(vals.dropFirst()) : vals))
                } else if vals.isEmpty {
                    payload = .unit
                } else {
                    payload = .tuple(vals.count > 1 ? Array(vals.dropFirst()) : vals)
                }
                let bareName = Self.nativeBareNameFromQualified(name)
                let kindName = bareName + "Kind"
                var tgPayload = payload
                if bareName == "MirRvalue" && idx == 1 {
                    if case .tuple(let parts) = payload, parts.count >= 2 { tgPayload = parts[1] }
                    else if case .structVal(_, let f) = payload, let p = f["1"] { tgPayload = p }
                }
                let wrappedEnum = MirValue.enumVal(kindName, idx, tgPayload)
                switch bareName {
                case "MirStatement":
                    return .structVal("MirStatement", ["kind": wrappedEnum, "span": Self.emptySpanValue])
                case "MirTerminator":
                    return .structVal("MirTerminator", ["kind": wrappedEnum, "span": Self.emptySpanValue])
                case "MirOperand":
                    return .structVal("MirOperand", ["kind": wrappedEnum])
                case "MirRvalue":
                    return .structVal("MirRvalue", ["kind": wrappedEnum])
                default:
        ...
        case .discriminant(let place):
            let val = loadPlaceValue(place)
            if case .enumVal(_, let idx, _) = val {
                return .int(idx)
            }
            // Handle struct-wrapped enums (MirStatement, MirTerminator, etc.) -
            // the kind field contains the inner enum whose idx is the discriminant.
            if case .structVal(_, let fields) = val, let kindVal = fields["kind"],
               case .enumVal(_, let idx, _) = kindVal {
                return .int(idx)
            }
    ...
    private static let nativeFastPathNames: Set<String> = [
        ...
        "buf_pos", "buf_new", "patch32_le", "align_to", "rex", "modrm", "sib", "span_new",
    ...
    private func dispatchCall(_ name: String, args: [MirValue], staticSite: CallSiteKey? = nil) -> MirValue {
        ...
        case "buf_new", "tg_compiler::asm::buf_new":
            // Return CodeBuffer initialized with MirByteBuffer (not array)
            // so that mutations through &mut borrows share the same buffer instance.
            return .structVal("CodeBuffer", [
                "bytes": .byteBuffer(MirByteBuffer()),
                "labels": .structVal("Map", ["entries": .array([]), "_nid": .int(0)]),
                "fixups": .array([]),
                "symbols": .array([]),
                "relocations": .array([])
            ])
