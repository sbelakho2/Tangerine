...
| UnsafeSlice[T] (the RAW view — the explicit unsafe/FFI form) | Inline (16-byte fat value {ptr@0, len@8}) | 16 | none — the non-owning borrowed view | ptr@0, len@8 | none — borrowed, trivially copyable, no drop | bit | none | the RAW view (the registered builtin, the LangItems slice id; slice_view_layout); the ONLY source spelling of the 16-byte {ptr,len} form — the compiler-scoped loop iterables / io SliceMut are the other raw forms. The SOURCE-spelled Slice[T] is the shared/pinned form below |
| Slice[T] (the SAFE Slice — the SOURCE-spelled shared/pinned form) | 8-byte Arc-class handle { inner: ArcStrong[SliceBacking[T]] } | 8 | 24 | the pinned backing SliceBacking[T] = { ptr@0, len@8, offset@16 } | shared — the Arc keeps the pinned backing alive; the view SURVIVES its source's drop | the Arc-class clone | the Arc drop | the ONE safe-view authority (std/collections.tg; memory_model.md §9) — the source-spelled Slice[T] (and the [T] sugar) IS this shared/pinned form; no escape tracking is needed (the backing owns) |
