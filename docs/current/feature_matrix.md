...
*Last updated: 2026-08 · working tree commit 6f1005a + the round-11 work
(Arc rework — unique-only get_mut, sink-consuming try_unwrap,
drop_in_place on the last-release path, refcount overflow guard;
DriverKind::Mysql; TLS consuming sink builders + the native tls shim lane;
extern-ABI return normalization; the SliceMut[u8] Read contract; the
Linux-AArch64 syscall translation table; the explicit LSE target contract;
the pthread opaque alignment override; the thread spawn failure-path glue;
docs reconciled with the executable state). No ladder/CI run has occurred
on this tree — no feature is implemented+run-verified at SHA.*
