...
- The specimens: `tests/grammar_f/` — for every production, four
  ...
  `neg/<prod>_before.tg`, `neg/<prod>_after.tg`.
- The negative specimens are the PRODUCTION-SPECIFIC malformations
  (the reviewer's item 10 strengthening): each production's negatives
  are the production's OWN grammar violations from the generator's
  per-production catalog — the wrong arity, the wrong keyword, the
  missing clause — placed BEFORE / AFTER the valid construct, so the
  parser must still reject the program (a production that swallows an
  error — or the rest of the file — fails). The generic stray-paren
  negative is FORBIDDEN: the gate mechanically backstops it (a bare
  `)` line in any negative specimen is a failure) and verifies the
  malformation block's position relative to the construct marker.
- The gate: `scripts/check_grammar_f_gate.sh` — facts↔files parity,
  regenerate-then-diff parity, the positional malformation structure,
  and — when a current-grammar compiler binary exists — the compiler
  verdicts (positives pass, negatives produce a diagnostic).
- The RELEASE CONTEXT (`scripts/check_grammar_f_gate.sh --release`):
  the compiler-verdict layer is REQUIRED — the release proof's Gate-F
  row carries the verdicts from the ACTUAL compiler execution, and
  without a usable current-grammar stage binary the row reads
  **PENDING-UNTIL-BINARY** (a FAIL — the structural pass is never a
  release pass) until the ladder produces one.
