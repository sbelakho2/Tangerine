...
| 14 | Code generation | `compile_lir_route (default) / generate_object_file / generate_executable (--codegen=direct)` | `driver.tg (route) / lir.tg (default) / codegen.tg (direct)` | route-selected instruction representation: LIR (the default host route) / the register-targeted direct emission (--codegen=direct); both share the layout/Repr table + IntrinsicId dispatch and are preceded by the final firewall (verify_mir + run_mir_completeness_oracle) | the route selected at compile entry is the instruction-selection authority. On the DEFAULT host route the LIR pipeline (driver.tg compile_lir_route — canonical default since fourth-audit P0-21) lowers MirProgram through tg_compiler/lir.tg's standalone linear-scan allocator (lir_alloc_registers_mode / lir_linearize — interval segments, pool registers, spill-slot assignment), then the MANDATORY post-allocation verify_lir gate, then the per-backend emission. generate_object_file/generate_executable are the authorities for the EXPLICIT --codegen=direct debug/bootstrap fallback: the register-targeted DIRECT EMISSION (the inline per-function register state RegAllocState — alloc_reg / free_reg / alloc_reg_or_spill — NOT a standalone register-allocation pass), the layout/Repr table (layout_engine.tg), and intrinsic dispatch (IntrinsicId in codegen.tg). The deprecated TANGERINE_LIR=1 alias no longer selects the route (the default is Lir; an explicit --codegen wins). The LIR route fails closed on any construct outside its legalization contract — never a silent per-function fallback | linker, runtime |
...
## The route-selection fact (the fourth-audit P0-21 flip)

The DEFAULT host compile route is the LIR pipeline (tg_compiler/lir.tg): MirProgram -> LIR -> the standalone LINEAR-SCAN register allocator (lir_alloc_registers_mode / lir_linearize — interval segments, pool registers, spill-slot assignment) -> the MANDATORY post-allocation verify_lir gate every LIR function runs -> per-backend emission -> object/link (driver.tg compile_lir_route; the codegen_default() flip point in compiler_core.tg made Lir canonical at fourth-audit P0-21). The DIRECT route (codegen.tg's register-targeted emission with the inline per-function RegAllocState — alloc_reg / free_reg / alloc_reg_or_spill; NO standalone allocation pass) is the EXPLICIT --codegen=direct debug/bootstrap fallback. TANGERINE_LIR=1 is a DEPRECATED LEGACY ALIAS: the CLI parser maps it onto the option only when the compile carries no explicit --codegen flag, and since Lir is the default it selects nothing new (an explicit --codegen wins). The LIR route FAILS CLOSED on any construct outside its legalization contract — never a silent per-function fallback to direct.
