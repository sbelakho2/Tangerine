...
| `alloc_policy` | native | `tests/unit/test_alloc_policy.tg` — the allocator-policy contract (audit P1-12): the AllocPolicy markers (Heap/Arena/Slab/Fixed/None) with the hot-path mapping (matching loop = None/Fixed, per-batch = Arena, per-symbol = Slab, embedded = None+Fixed+with_buffer), the deterministic BumpArena (owned new/new_aligned, borrowed with_buffer/with_buffer_aligned; alignment-honoring alloc/alloc_layout/reset over power-of-two aligns through the 64-byte anchor; zero-overhead reset; null-on-exhaustion), the SlabPool fixed-size typed pool (preallocated slots aligned to the element type, free-list acquire/release index hand-off, full-pool Err(item) rejection, no-op double release, exact ascending teardown of live occupants at clear/Drop, no heap churn after init) and the ArenaVec[T: Copy] arena-backed builder (push/get/len/reset, zero general-heap calls after construction) — BumpArena/SlabPool/ArenaVec are classified non-Copy by the type-property engine (registered Drop, pinned by the tests/canary_neg copy canaries); test_alloc_policy.tg wires the slab-funded ring consumer and asserts alignment honors, reset determinism, exact once-only release and the no-heap-after-init property through the allocator's outstanding-bytes counter; allocator-parameterized Vec[T, A]/Map forms are documented future work (Vec growth is the __intrinsic_array_push intrinsic, kernel-side work) |
...
| `embedded` | experimental | `tests/run_stdlib_e106_sweep.sh`; `tests/embedded/embedded_mmio_behavior_test.tg` — the volatile/MMIO Register abstraction, the bitfield helpers, the allocator-free ArrayVec/RingBuffer and the atomicity availability are host-tested; the bare-metal route (P0.2): aarch64-unknown-none is the REAL target of the --target embedded route (the aarch64 backend, the spec/linker/startup artifacts + the bare-metal ELF image); the desc'd Thumb/RISC-V triples (thumbv7m-none-eabi / thumbv7em-none-eabi[f] / riscv32imac/riscv32imafc/riscv32imafdc/riscv64imac/riscv64gc-unknown-none-elf) ROUTE to the LIR pipeline for the descriptor the --target triple names (target_desc_of_triple — ELF32 ARM / ELF32\|ELF64 RISC-V objects), while the triples with no TargetDesc instance (thumbv6m-none-eabi / thumbv8m.main-none-eabihf / riscv32imc-unknown-none-elf) stay HARD-REJECTED — the stable diagnostic, no artifact; the HOST default is the LIR pipeline since fourth-audit P0-21 (--codegen=direct selects the explicit debug/bootstrap fallback), and the deprecated TANGERINE_LIR=1 alias selects nothing new — TANGERINE_LIR_TARGET remains a legacy alias key of the same table (driver.tg compile_lir_route) |
...
| `fixed` | native | `tests/unit/test_fixed_containers.tg` — the fixed-capacity collections (ArrayVec/RingBuffer/SpscQueue with split endpoints): the ownership-transferring push/pop semantics, the full-queue rejection that returns the item unconsumed, the ring wrap through the mask fast path, and the SPSC round trip (test_fixed_containers.tg). The module carries BOTH compile-time-capacity surfaces (std/fixed.tg's module doc): the capacity-named concrete families (ArrayVecN[T] / RingBufferN[T], the split-endpoint SpscQueueN[T]/SpscTxN[T]/SpscRxN[T]) — the original core-quality engine, kept for the stage0-subset compatibility — and the CONST-GENERIC VIEWS ArrayVec[T, const N: UInt] / RingBuffer[T, const N: UInt], the promoted surface for new code since the language gained const generics (§20: Type::Const / Type::ConstParam / Type::ConstSym, value-level reads of the parameter in generic bodies, impl-level const parameters); std/embedded.tg spells the same const-generic ArrayVec/RingBuffer for the no_alloc environments |
...
| `semantic_diff` | parse-clean | `tests/run_stdlib_e106_sweep.sh` — TEXTUAL/heuristic line-based source-diff utility (audit §28 + §45): presentation only — NOT a semantic authority; semantic identity/diff is tg_compiler's semantic_delta.tg + semantic snapshot architecture (semantic_model.tg / semantic_snapshot.tg / semantic_query.tg). Never gate semantic-equivalence, ABI, effects or ownership claims on this module. |
