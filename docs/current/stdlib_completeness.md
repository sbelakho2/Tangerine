...
`std/*.tg` glob — never typed. This tree enumerates **131 modules**
...
**new** `std/*.tg` file (a 132th module) has no contract, and the
...
| `experimental` | parse-clean only; the module is flagged experimental (item 33 stable-subset policy) and is EXPLICITLY EXCLUDED from the shipped-std behavior claims | 17 |
...
| `android` | experimental | `tests/run_stdlib_e106_sweep.sh` |
...
| `cocoa` | experimental | `tests/run_stdlib_e106_sweep.sh` |
...
| `embedded` | experimental | `tests/run_stdlib_e106_sweep.sh` |
...
| `gpu` | experimental | `tests/run_stdlib_e106_sweep.sh` |
...
| `gui` | experimental | `tests/run_stdlib_e106_sweep.sh` |
| `hal` | experimental | `tests/run_stdlib_e106_sweep.sh` — committed host-side rigor artifact(s) exist but are non-claiming for the unserved platform target |
...
| `ios` | experimental | `tests/run_stdlib_e106_sweep.sh` |
| `json` | parse-clean | `tests/run_stdlib_e106_sweep.sh` |
| `kernel` | experimental | `tests/run_stdlib_e106_sweep.sh` |
...
| `windows` | experimental | `tests/run_stdlib_e106_sweep.sh` |
...
2. **The stable-subset policy (CHOSEN)** — the experimental modules are
   ...
   The claim covers the 114 non-experimental modules; the 17 experimental
...
**The affected modules (17):** `android`, `cocoa`, `effects`, `embedded`, `gfx_gpu`, `gpu`, `gpu_metal`, `gpu_vulkan`, `gpu_webgpu`, `gui`, `hal`, `ios`, `kernel`, `simd`, `wasm`, `wasm_js`, `windows`.
