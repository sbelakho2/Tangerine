...
| `embedded` | experimental | `tests/run_stdlib_e106_sweep.sh`; `tests/embedded/embedded_mmio_behavior_test.tg` — the volatile/MMIO Register abstraction, the bitfield helpers, the allocator-free ArrayVec/RingBuffer and the atomicity availability are host-tested; the bare-metal route (P0.2): aarch64-unknown-none is the REAL target of the `--target` embedded route (the aarch64 backend); the Thumb (thumbv6m/thumbv7em/thumbv8m.main) and RISC-V (riscv32imc/riscv32imac/riscv64gc) triples are HARD-REJECTED on that route — the stable diagnostic, no artifact; the ARMv7-M/RISC-V code generators exist on the env-gated LIR route instead (`TANGERINE_LIR=1` + `TANGERINE_LIR_TARGET=cortex-m3/cortex-m4/cortex-m4f/cortex-m7` or `riscv32imac/riscv32imafc/riscv32imafdc/riscv64imac/riscv64gc` — driver.tg `compile_lir_route`, default off, pending full-corpus parity) |
