...
| THE SIMD VECTOR ROW: the std Vec2/Vec4/Vec8/Vec16 instantiations with a 16/32/64-byte width lay out as N lanes of the lane type with ALIGNMENT = the vector WIDTH (never the lane alignment), and the ABI classifier's vector cases cross the 16-byte vector THROUGH the registers (`Vector(16)` — never sret); the 32-byte form crosses by address under the desc-free category rule and is promoted to the `Vector(32)` ymm crossing by the call plan exactly where the descriptor models the 256-bit class (the plan-level v256 promotion — an x86-64 desc with the avx/avx2 tokens), while the 64-byte forms cross by address (no descriptor models 512 bits); a non-vector width (Vec2[f32] — 8 bytes) and an ordinary `[T; N]` fixed array keep the element-aligned inline layout | layout_engine.tg `compute_vector_layout` / `is_simd_vector_type` / `classify_value_category` / `abi_plan_vector_promote` / `vector_crossing_legal`; codegen.tg `AbiArg::Vector` / `category_uses_sret`; tests/simd/simd_layout_test.tg / simd_abi_probe.tg |
