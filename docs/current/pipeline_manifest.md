...
| 14 | Code generation | `compile_lir_route` (default) / `generate_object_file` / `generate_executable` (`--codegen=direct`) | driver.tg (route) / lir.tg (default) / codegen.tg (direct) | route-selected: the DEFAULT host route is the LIR pipeline (`lir.tg` — MIR → LIR → standalone linear-scan allocation → the mandatory post-allocation `verify_lir` gate → per-backend emission; canonical since fourth-audit P0-21); `--codegen=direct` selects codegen.tg's direct emitter — the explicit debug/bootstrap fallback (the inline per-function `RegAllocState`; no standalone allocation pass); preceded by the final firewall (verify_mir + `run_mir_completeness_oracle`) |
...
| 14 | Code generation | the route selected at compile entry is the instruction-selection authority. The DEFAULT host route is the LIR pipeline (driver.tg `compile_lir_route` — canonical since fourth-audit P0-21): MirProgram → LIR → the standalone linear-scan allocator in lir.tg (`lir_alloc_registers_mode`/`lir_linearize`) → the MANDATORY post-allocation `verify_lir` gate → per-backend emission. `generate_object_file`/`generate_executable` are the authorities for the EXPLICIT `--codegen=direct` debug/bootstrap fallback: the register-targeted DIRECT EMISSION (the inline per-function register state `RegAllocState` — `alloc_reg`/`free_reg`/`alloc_reg_or_spill` in codegen.tg; no standalone register-allocation pass), plus the layout/`Repr` table (layout_engine.tg) and intrinsic dispatch (`IntrinsicId` in codegen.tg) shared by both routes. The deprecated `TANGERINE_LIR=1` alias no longer selects the route (the default is Lir; an explicit `--codegen` wins); the LIR route fails closed outside its legalization contract | linker, runtime |
