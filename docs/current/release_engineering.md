...
| `algebraic-effects` | api-only | excluded-from-release | declared surface never lowered (no effect-log runtime; the non-budget effect-record arm is a lowering-defect fail-closed trap); the `effects` surface is gated experimental |
