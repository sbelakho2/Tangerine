...
| `algebraic-effects` | api-only | **implemented** | the typed effect row lower end to end: the declarations' `TypedEffectDecl` op tables (`tg_compiler/types.tg`), MIR `MirEffectPerform` / `MirEffectFrameEnter` / `MirEffectFrameExit` (`tg_compiler/mir.tg`), the `__tg_effect_*` runtime arms and the direct-style handler stack (`tg_compiler/codegen.tg`, `tg_compiler/runtime.tg`), with `std/effects.tg`; `tests/effects/` (state/reperform/unhandled/async); the bounded row stays gated experimental (Copy-payload only, no handler across an async task boundary) |
...
| `gpu-platform` | api-only | **implemented** | the driver-side `GpuBackend` abstraction plus the software/reference backend (device enumeration, buffer create/upload/download, `ComputeKernel::create`/`set_buffer`/`dispatch`, the shader-compile fail-closed path) in `std/gpu.tg`; the hardware backends (Vulkan/Metal/CUDA/OpenCL) probe the real library via dynload and fail closed when absent; `tests/gpu/gpu_software_backend_test.tg` + `tests/hal/hal_software_backend_test.tg`; gated experimental |
| `platform-only-surfaces` | api-only | **implemented** | the real FFI bindings + smoke tests: cocoa's ObjC round trip, windows' wide-string/user32-kernel32 sets, android's JNI surface, ios' platform helpers, and gui's software `CanvasBuffer` rasterizer (`std/gui.tg`); `tests/platform/platform_surface_smoke_test.tg` + `tests/gui/gui_software_canvas_test.tg`; gated experimental |
| `kernel-primitives` | api-only | **implemented** | the alpha stubs completed in `std/kernel.tg`: the round-robin scheduler table (`Scheduler::tick`), the CAS-backed mutex, the counting semaphore, the fixed PCB table and the host process spawn bridge; `tests/kernel/kernel_primitives_test.tg`; gated experimental |
| `non-kernel-stdlib-behavior` | api-only | **implemented** | the real behavior suites over the parse-clean modules' pure OS-independent surfaces (semver, url percent round trip, csv parse, unicode classes, hash determinism); `tests/nonkernel_stdlib_behavior_test.tg` |
| `packed-align-attributes` | design | **implemented** | the attributed layout rules end to end: `parser.tg extract_layout_attrs` (E109 arg shape + exclusivity), `types.tg validate_layout_attrs`, `layout_engine.tg compute_struct_layout`/`compute_enum_layout` + `c_aggregate_category`, and the byte-wise unaligned access lowering in `codegen.tg`; `tests/layout/packed_align_test.tg`, `tests/layout/packed_align_engine_test.tg`, `tests/layout/differential_layout_test.tg` |
| `real-time-wcet` | design | **implemented** | the `@[real_time(wcet_us=N)]` analyzer: the annotation parsed and validated, the static worst-case cost estimated (the operation cost model, transitive callee costs, count-annotated loop bounds) and an over-bound body rejected with the hard validity error (`types.tg parse_real_time_attr` / `estimate_wcet_cost`); `tests/wcet_analyzer_test.tg` |
...
| `no-std` | unsupported | **implemented** | the `@no_std` attribute / `--no-std` flag with the core-only import surface and the `def _start() -> !` bare entry (`driver.tg compile_to_nostd_route` — `nostd_surface_error` / `start_entry_error`, a `main` is rejected) plus the linker's default → `_start` → `_reset_handler` fallback (`linker.tg linker_resolve_entry_name`); `tests/nostd/nostd_bare_start.tg`, `tests/nostd/nostd_core_surface_negative.tg`, `tests/run_nostd_bare_lane.sh` |
| `wasi` | unsupported | **implemented** | the `std::wasi` guest surface plus the wasm-target import wiring (`add_wasi_imports` — `wasi_snapshot_preview1`) and the host-side ABI (`runtime.tg emit_wasi_host_runtime` — the iovec walk, errno normalization, clock_gettime-backed clock); `tests/wasi/` + `tests/run_wasi_conformance.sh` (the wasmtime `--dir` lane runs when the runtime is installed); gated experimental |
| `embedded-targets` | unsupported | **implemented** | the `--target` embedded route: `driver.tg compile_to_embedded_route` (the target spec/linker script/startup/vector artifacts + the bare-metal aarch64 ELF image) and `compile_lir_route` for the desc'd Thumb/RISC-V triples (`target_desc.tg`'s ONE `target_desc_of_triple` lookup; ELF32 ARM / ELF32\|ELF64 RISC-V objects, executable linking fails closed; desc-less triples HARD-REJECTED); `tests/embedded/` + `tests/run_embedded_lane.sh`; no QEMU execution lane (hardware execution not claimed); gated experimental |
