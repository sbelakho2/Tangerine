...
let vm_inspect (prog : Seed_mir.program) : (string, string) result =
  match
    Vm.entry_frame_of_li ~limits:Vm.default_limits ~lang_items:Lang_items.seed_defaults ~program:prog
...
let check_vm () =
  ...
  in
  (match Vm.entry_frame_of_li ~limits:Vm.default_limits ~lang_items:Lang_items.seed_defaults ~program:drop_after_move
