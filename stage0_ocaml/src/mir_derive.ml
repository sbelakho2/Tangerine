...
let rec emit_clone_value (env : Typecheck.env) (s : st)
  ...
  (* NOMINAL IDENTITY (Box-transparency regression fix): Box[T] is a
     component with its own representation and its own registered
     `impl[T: Clone] Clone for Box[T]` (std/alloc.tg).  resolve_clone_impl
     below resolves that impl exactly like any other nominal's — the
     derived clone never clones the boxed content in place. *)
