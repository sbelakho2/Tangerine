...
the tree):
  The stage2 == stage3 byte-identity verdict, the per-phase fingerprints
  (link-image / text / sections / symbols / relocs + the probed token/ast/
  hir/mir/mir-mono dumps) and the stage binaries' hashes live in the
  bootstrap job's logs and the uploaded bootstrap-fingerprints /
  bootstrap-native-tests artifacts (hashed above). A source tree alone
  cannot produce them — a snapshot that lacks them has no run evidence.
