...
# Usage: bh_dump_hash <compiler> <source> <flag>
bh_dump_hash() {
  local compiler="$1" source_file="$2" flag="$3"
  "$compiler" compile "$source_file" "$flag" 2>/dev/null | bh_sha256_str
}

# The eight compiler pipeline phases fingerprinted per stage.
# Each maps to a deterministic compiler emission or artifact:
#   tokens         : sha256 of the token stream dump        (--print-tokens)
#   ast            : sha256 of the AST dump                 (--print-ast)
#   resolved-ast   : sha256 of the AST dump as the merged/resolved view
#                    seen by the frontend (same dump source; distinct phase)
#   mir-lowered    : sha256 of the post-lowering MIR        (--emit-mir, O0)
#   mir-mono       : sha256 of the post-monomorphization MIR (--emit-mir, O0)
#   mir-opt        : sha256 of the post-optimization MIR    (--emit-mir, O1)
...
bh_fingerprints() {
  ...
  if "$compiler" compile "$source_file" -o "$obj_file" -c --target aarch64-apple-darwin >/dev/null 2>&1; then
    ...
    obj_hash="UNEMITTED"
    bh_warn "object emission failed for $stage; fingerprint recorded as UNEMITTED"
  fi
...
run_native_tests() {
  ...
  for dir in "${dirs[@]}"; do
    ...
    for file in "$dir"/*.tg; do
      ...
      bh_log "native test: $file"
      if ! "$compiler" compile "$file" -o "$bin" --target aarch64-apple-darwin >/dev/null 2>&1; then
        bh_err "native test $name failed to compile"
        failures=$((failures + 1))
        continue
      fi
      if ! bh_is_macho64 "$bin"; then
        bh_err "native test $name produced non-Mach-O output"
        failures=$((failures + 1))
        continue
      fi
      chmod +x "$bin"
      ...
      rc=$?
      if [ "$rc" -ne 0 ]; then
        bh_err "native test $name exited nonzero ($rc)"
        failures=$((failures + 1))
      fi
    done
  done

  bh_log "native tests: $((total - failures))/$total passed"
  if [ "$failures" -ne 0 ]; then
    bh_err "native tests FAILED: $failures failure(s)"
    return 1
  fi
  bh_log "native tests OK"
  return 0
}
...
# compiler is deterministic under identical inputs (reproducible build).
#
# Usage: check_two_clean_dirs <stage-name> <compiler> <source-copy-a> <source-copy-b> <outdir>
check_two_clean_dirs() {
  local stage="$1" compiler="$2" srca="$3" srcb="$4" outdir="$5"
  mkdir -p "$outdir"

  local out_a="$outdir/${stage}_a" out_b="$outdir/${stage}_b"

  bh_log "$stage determinism: building from clean dir A"
  if ! "$compiler" compile "$srca" -o "$out_a" --target aarch64-apple-darwin >/dev/null 2>&1; then
    bh_err "$stage determinism: build A failed"
    return 1
  fi

  bh_log "$stage determinism: building from clean dir B"
  if ! "$compiler" compile "$srcb" -o "$out_b" --target aarch64-apple-darwin >/dev/null 2>&1; then
  ...
  if [ "$ha" != "$hb" ]; then
    bh_err "$stage determinism FAILED: clean-dir builds differ"
    return 1
  fi

  bh_log "$stage determinism OK (byte-identical across two clean dirs)"
  return 0
}
