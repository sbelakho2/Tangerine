...
p_lines.append("")
p_lines.append("Stage 14 (codegen) is the DEFAULT route's DIRECT emission with the")
p_lines.append("inline per-function register state (`RegAllocState` — `alloc_reg` /")
p_lines.append("`free_reg` / `alloc_reg_or_spill`, codegen.tg) — no standalone")
p_lines.append("register-allocation pass on that route. A standalone LINEAR-SCAN")
p_lines.append("register-allocation pipeline exists behind the env-gated LIR route")
p_lines.append("(`TANGERINE_LIR=1`, default off — driver.tg `compile_lir_route`):")
p_lines.append("MirProgram -> LIR -> linear scan (`lir.tg` `lir_alloc_registers_mode`")
p_lines.append("/ `lir_linearize` — interval segments, pool registers, spill-slot")
p_lines.append("assignment) -> per-backend emission. The LIR route is NOT the")
p_lines.append("default: it is an env opt-in pending full-corpus parity with the")
p_lines.append("direct emitter, and it fails closed on any construct outside its")
p_lines.append("legalization contract. Un-gated pipeline prose means the direct")
p_lines.append("route's register-targeted emission.")
p_lines.append("")
p_lines.append("---")
p_lines.append("")
