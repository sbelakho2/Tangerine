...
let rec emit_expr ctx expr =
  ...
  | EBinOp (Add, left, right, _) ->
    ...
    if is_string_type lt || is_string_type rt then begin
      ...
      if not (is_string_type lt) then begin
        emit ctx "tg_int_to_string("; emit_expr ctx left; emit ctx ")"
      end else
        emit_expr ctx left;
      emit ctx ", ";
      if not (is_string_type rt) then begin
        emit ctx "tg_int_to_string("; emit_expr ctx right; emit ctx ")"
      end else
        emit_expr ctx right;
      emit ctx ")"
    end else begin
  ...
  | ECast (inner, typ, _) ->
    (match typ with
     ...
     | TyName ("String", []) -> emit ctx "tg_int_to_string("; emit_expr ctx inner; emit ctx ")"
